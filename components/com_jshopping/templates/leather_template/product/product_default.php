...
?>
<div class="jshop_mystyle one_product_one_page jshop_mystyle col-md-10 col-sm-9 col-xs-12">
	<div class="jshop productfull row-fluid" id="comjshop">
		<form name="product" method="post" action="<?php print $this->action ?>" enctype="multipart/form-data"
			...
			<div class="jshop image_middle_wrap">
					...
					<?php if ($this->config->product_show_manufacturer_logo && $this->product->manufacturer_info->manufacturer_logo != "") { ?>
						<div class="manufacturer_logo">
							<a href="<?php print SEFLink('index.php?option=com_jshopping&controller=manufacturer&task=view&manufacturer_id=' . $this->product->product_manufacturer_id, 2); ?>">
								<img
									src="<?php print $this->config->image_manufs_live_path . "/" . $this->product->manufacturer_info->manufacturer_logo ?>"
			...
			<div class="wrapper_description_product">
						...
						<?php foreach ($this->product->freeattributes as $freeattribut) { ?>
							<div class="row-fluid">
								<div class="span2 name">
									...
									<span
										class="freeattribut_description"><?php print $freeattribut->description; ?></span>
								</div>
						...
						<?php foreach ($this->product->product_add_prices as $k => $add_price) { ?>
                            ...
                            <span id="pricelist_from_<?php print $add_price->product_quantity_start ?>">
                                <?php print formatprice($add_price->price) ?><?php print $add_price->ext_price ?>
									...
									<span
										class="per_piece">/ <?php print $this->product->product_add_price_unit ?></span>
								</td>
							</tr>
						<?php } ?>
					</table>
					...
					<div class="prod_buttons" style="<?php print $this->displaybuttons ?>">
						...
						<div class="prod_qty_input">
							<input type="number" name="quantity" id="quantity" onkeyup="reloadPrices();"
							       class="inputbox"
							       min="1" value="<?php print $this->default_count_product ?>"/>
							<?php print $this->_tmp_qty_unit; ?>
						</div>


						<div class="buttons">
							<input type="submit" class="button_incart"
							       value="<?php print _JSHOP_ADD_TO_CART ?>" onclick="jQuery('#to').val('cart');"/>
				...
				<?php if ($this->config->product_show_button_back) { ?>
					<div class="button_back">
						<input type="button" class="" value="<?php print _JSHOP_BACK; ?>"
						       onclick="<?php print $this->product->button_back_js_click; ?>"/>
					</div>
				<?php } ?>
				<div class="hidden_alert_error">
					Выберите параметры!
				</div>
			</div>
