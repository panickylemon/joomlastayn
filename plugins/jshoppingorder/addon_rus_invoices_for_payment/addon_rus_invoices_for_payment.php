...
// грузополучатель и чё то там ещё
...
//		$pdf->text(13,98,_JSHOP_ADDON_RUS_INVOICES_PAYMENT_CUSTOMER.implode(", ",$adress));
	...
	//сумма всего текстом
		$pdf->SetFont('freesans','',8);
		$pdf->SetXY(13,$y);
		$pdf->MultiCell(180, 1 , _JSHOP_ADDON_RUS_INVOICES_PAYMENT_TOTAL_ITEMS.$num.", "
				._JSHOP_ADDON_RUS_INVOICES_PAYMENT_THE_AMOUNT.$this->formatprice($order->order_total)." руб.",'0','L');
		$pdf->SetFont('freesansb','',8);
		$pdf->SetXY(13,$y+5);
...
//		$pdf->text(14,$y,$text);
	...
	function addTitleHead(&$pdf){
		...
		$pdf->Image($jshopConfig->path.'images/header.jpg',170,12,$jshopConfig->pdf_header_width,
